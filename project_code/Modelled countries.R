...
WEO <- cbind(WEO[rep(seq_len(nrow(WEO)), length(pov.lines))], PL=rep(pov.lines, each=nrow(WEO)))

WEO.melt <- melt(WEO, id.vars=c("ISO", "CountryName", "year", "mean", "ppp", "type", "A", "B", "C", "PL"))
...
hc <- merge(countries_GQ[,c("ISO", "CountryName", "variable", "PL", "GQP1", "GQP2")], countries_Beta[,c("ISO", "CountryName", "PL", "variable", "BetaP1")])
hc[is.na(hc)] <- -1
